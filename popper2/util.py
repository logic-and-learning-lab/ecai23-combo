...
class Settings:
    def __init__(self, ex_file=None, bk_file=None, bias_file=None, kbpath=False, info=False, debug=False, show_stats=False, bkcons=False, max_literals=MAX_LITERALS, timeout=TIMEOUT, eval_timeout=EVAL_TIMEOUT, max_examples=MAX_EXAMPLES, max_body=MAX_BODY, max_rules=MAX_RULES, max_vars=MAX_VARS, functional_test=False):

        if ex_file != None and bk_file != None and bias_file != None:
            self.bk_file, self.ex_file, self.bias_file = bk_file, ex_file, bias_file
        elif kbpath != False:
